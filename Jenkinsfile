    ...
    post {
        ...
        failure {
            ...
            withCredentials([string(credentialsId: 'TG', variable: 'TOKEN')]) {
                sh  ("""
                    curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendMessage -d chat_id=${TG_CHAT_ID} -d parse_mode=markdown -d text='*${JOB_NAME}*; *Build* : FAILED; ${JENKINS_URL}/final_task_learn/${BUILD_NUMBER}'
                    """)
            }
            //TG notification
        }
